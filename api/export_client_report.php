...
// ══════════════════════════════════════════════════════════════════════════════
// SHEET 5: Conformance Score — calculate <v> values from Final Report data
// Columns: A=SC Name, B=SC Number, C=Count, D=User Affected,
//          E=Blocker, F=Critical, G=Major, H=Minor, I=Pass/Fail
// Formulas reference 'Final Report'!$O:$O (WCAG SC) and $L:$L (Severity)
// We pre-calculate and update <v> so Excel shows correct values immediately.
// ══════════════════════════════════════════════════════════════════════════════
$sh5 = $zip->getFromName('xl/worksheets/sheet5.xml');
if ($sh5 !== false) {
    // Build lookup: SC number/name → [total, blocker, critical, major, minor]
    // Final Report col O = wcagNums (comma-separated), col L = severity
    $scStats = []; // key = lowercase SC name or number
    foreach ($filteredIssues as $iss) {
        $iid  = (int)$iss['id'];
        $meta = $metaMap[$iid] ?? [];
        $sev  = strtolower(trim(metaFirst($meta, 'severity') ?: ($iss['severity'] ?? 'minor')));
        $scNums  = metaArray($meta, 'wcagsuccesscriteria');
        $scNames = metaArray($meta, 'wcagsuccesscriterianame');
        // Each SC number this issue affects
        foreach ($scNums as $idx => $scNum) {
            $scNum  = trim($scNum);  if ($scNum === '') continue;
            $scName = trim($scNames[$idx] ?? '');
            // Index by both number and name for flexible lookup
            foreach (array_filter([$scNum, $scName]) as $key) {
                $k = strtolower($key);
                if (!isset($scStats[$k])) $scStats[$k] = ['total'=>0,'blocker'=>0,'critical'=>0,'major'=>0,'minor'=>0,'low'=>0];
                $scStats[$k]['total']++;
                if (isset($scStats[$k][$sev])) $scStats[$k][$sev]++;
            }
        }
    }

    // Parse sheet5 shared strings to get SC names/numbers from col A and B
    // Then update <v> in formula cells E, F, G, H, I per row
    // We use a regex to find each row, read col A/B shared string index, compute values, update <v>
    $ssRaw = $zip->getFromName('xl/sharedStrings.xml');
    $sharedStrings = [];
    if ($ssRaw !== false) {
        preg_match_all('/<si>.*?<t[^>]*>([^<]*)<\/t>.*?<\/si>/s', $ssRaw, $ssm);
        $sharedStrings = $ssm[1] ?? [];
    }

    // Helper: get shared string value by index
    $getStr = function(string $idx) use ($sharedStrings): string {
        $i = (int)$idx;
        return $sharedStrings[$i] ?? '';
    };

    // Update <v> in formula cells for each data row (row 2+)
    // Pattern: find <c r="X{row}" ...><f ...>...</f><v>old</v></c> and replace <v>
    $sh5 = preg_replace_callback(
        '/<row\s+r="(\d+)"[^>]*>(.*?)<\/row>/s',
        function($rowMatch) use ($getStr, $scStats) {
            $rowNum  = (int)$rowMatch[1];
            $rowXml  = $rowMatch[2];
            if ($rowNum < 2) return $rowMatch[0]; // skip header row

            // Get SC name from col A (shared string)
            $scName = '';
            if (preg_match('/<c\s+r="A' . $rowNum . '"[^>]*><v>(\d+)<\/v><\/c>/', $rowXml, $am)) {
                $scName = strtolower(trim($getStr($am[1])));
            }
            // Get SC number from col B (shared string)
            $scNum = '';
            if (preg_match('/<c\s+r="B' . $rowNum . '"[^>]*><v>(\d+)<\/v><\/c>/', $rowXml, $bm)) {
                $scNum = strtolower(trim($getStr($bm[1])));
            }

            // Look up stats — try SC name first, then number
            $stats = null;
            foreach ([$scName, $scNum] as $key) {
                if ($key !== '' && isset($scStats[$key])) { $stats = $scStats[$key]; break; }
            }
            if ($stats === null) $stats = ['total'=>0,'blocker'=>0,'critical'=>0,'major'=>0,'minor'=>0,'low'=>0];

            $total    = $stats['total'];
            $blocker  = $stats['blocker'];
            $critical = $stats['critical'];
            $major    = $stats['major'];
            $minor    = $stats['minor'] + ($stats['low'] ?? 0);
            $passFail = ($total === 0) ? 'Pass' : 'Fail';

            // Map: col C=total, E=Blocker, F=Critical, G=Major, H=Minor, I=Pass/Fail
            $updates = [
                'C' => (string)$total,
                'E' => (string)$blocker,
                'F' => (string)$critical,
            ...
            foreach ($updates as $col => $val) {
                $ref = $col . $rowNum;
                // Replace <v>...</v> inside the formula cell for this column
                $newRowXml = preg_replace(
                    '/(<c\s+r="' . preg_quote($ref, '/') . '"[^>]*>(?:<f[^>]*>.*?<\/f>))<v>[^<]*<\/v>/s',
                    '$1<v>' . htmlspecialchars($val, ENT_XML1, 'UTF-8') . '</v>',
                    $newRowXml, 1
                );
            }
            return str_replace($rowXml, $newRowXml, $rowMatch[0]);
        },
        $sh5
    );
