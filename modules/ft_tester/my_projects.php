...
$assignedProjectsQuery = "
    SELECT DISTINCT p.id, p.title, p.po_number, p.status, p.project_type,
           ...
           COUNT(DISTINCT CASE WHEN pe.ft_tester_id = ? OR JSON_CONTAINS(pe.ft_tester_ids, CAST(? AS JSON)) THEN pp.id END) as assigned_pages,
           COUNT(DISTINCT CASE WHEN pe.status = 'tested' AND (pe.ft_tester_id = ? OR JSON_CONTAINS(pe.ft_tester_ids, CAST(? AS JSON))) THEN pp.id END) as completed_pages
    FROM projects p
    ...
    WHERE (
        pe.ft_tester_id = ?
        OR pp.ft_tester_id = ?
        OR JSON_CONTAINS(pe.ft_tester_ids, CAST(? AS JSON))
        OR JSON_CONTAINS(pp.ft_tester_ids, CAST(? AS JSON))
    )
