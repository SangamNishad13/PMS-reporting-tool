...
$assignedProjectsQuery = "
    SELECT DISTINCT p.id, p.title, p.po_number, p.status, p.project_type,
           ...
           COUNT(DISTINCT CASE WHEN pe.ft_tester_id = ? OR JSON_CONTAINS(COALESCE(pe.ft_tester_ids,'[]'), ?) THEN pp.id END) as assigned_pages,
           COUNT(DISTINCT CASE WHEN pe.status = 'tested' AND (pe.ft_tester_id = ? OR JSON_CONTAINS(COALESCE(pe.ft_tester_ids,'[]'), ?)) THEN pp.id END) as completed_pages
    FROM projects p
    ...
    WHERE (
        pe.ft_tester_id = ?
        OR pp.ft_tester_id = ?
        OR JSON_CONTAINS(COALESCE(pe.ft_tester_ids,'[]'), ?)
        OR JSON_CONTAINS(COALESCE(pp.ft_tester_ids,'[]'), ?)
    )
